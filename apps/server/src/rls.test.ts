...
test("what an agent asks a pull request to say belongs to its run's organization", async () => {
  ...
  await pool.query(
    // A run says which board it is on: the column lost its default, so
    // a card's run has to name itself one rather than be filed as one.
    `insert into agent_runs (id,type,feature_id,organization_id,stage_id,agent_profile_id,prompt)
     values ($1,'pipeline',$2,'org-a',$3,$4,'work')`,
    [run, feature, stage, profile],
  );
