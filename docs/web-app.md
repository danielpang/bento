...
The mark itself is checked in five times, which is five chances to update four of them:
...
That last one is the one that had drifted: it was still the pre-bento mark, because the Native SDK decodes through the platform and cannot read SVG, so it is a raster nobody regenerated. The script draws the mark directly rather than rasterising it, since the one SVG renderer on a clean mac composites onto opaque white and an app icon needs transparent corners.
