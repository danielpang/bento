...
A stage with an assigned agent starts it automatically when a card arrives, including when the card is sent back, so a fully configured pipeline runs end to end on its own. Sending a card from Quality engineering back to Implementation starts the Implementation agent, not the one that just ran QA.
...
An agent is a coding tool paired with a model, and a stage points at one. Add one under **Agents**, then assign it to a stage. Which tools exist and how each authenticates: [agents.md](./agents.md).
